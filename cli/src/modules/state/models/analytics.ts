...
export function createAnalyticsModel(db: Database.Database) {
  const insert = db.prepare(`
    INSERT INTO analytics (post_id, likes, retweets, replies, impressions)
    VALUES (@post_id, @likes, @retweets, @replies, @impressions)
  `);

  return {
    record(postId: number, stats: { likes: number; retweets: number; replies: number; impressions: number }): void {
      insert.run({ post_id: postId, ...stats });
    },
    ...
    getTopPosts(limit = 10): PostWithAnalytics[] {
      return db.prepare(`
        SELECT p.id, p.content, p.prompt_type, p.tweet_id, p.posted_at,
               a.likes, a.retweets, a.replies, a.impressions
        FROM posts p
        JOIN analytics a ON a.post_id = p.id
        WHERE p.status = 'posted'
        GROUP BY p.id
        HAVING a.checked_at = MAX(a.checked_at)
        ORDER BY (a.likes + a.retweets * 2 + a.replies * 3) DESC
        LIMIT ?
      `).all(limit) as PostWithAnalytics[];
    },

    getSummary(): { totalPosts: number; totalLikes: number; totalRetweets: number; totalReplies: number; totalImpressions: number; avgEngagement: number } {
      const row = db.prepare(`
        SELECT
          ...
          COALESCE(AVG(a.likes + a.retweets * 2 + a.replies * 3), 0) as avgEngagement
        FROM posts p
        LEFT JOIN (
