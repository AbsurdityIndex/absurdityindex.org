(function () {
  ...
  function safeUrl(value) {
    try {
      ...
      const parsed = new URL(url, 'https://absurdityindex.org');
  ...
  fetch('https://absurdityindex.org/api/bills.json')
    ...
    .then(function (data) {
      const bill = data.bills.find(function (b) {
        return b.id === billId;
      });
      ...
      const safeBillUrl = safeUrl(bill.url);
      const absurdityValue = Number(bill.absurdityIndex);
      container.replaceChildren();
      ...
      const billNo = document.createElement('span');
      ...
      if (Number.isFinite(absurdityValue) && absurdityValue > 0) {
        const absurdityBadge = document.createElement('span');
      ...
      if (bill.billType !== 'real') {
        ...
        const typeLabel = billTypeLabel(bill.billType);
